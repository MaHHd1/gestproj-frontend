  ...
  template: `
    <div class="p-8 max-w-3xl">
      <div class="mb-6">
          ...
          <p class="text-slate-400 text-sm mt-1">Your account and identity details</p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-[#161b22] border border-slate-700 rounded-2xl p-6">
          <div class="flex items-center gap-4 mb-6">
            ...
            <button (click)="refresh()" class="ml-auto rounded-lg border border-slate-600 px-3 py-2 text-xs font-medium text-slate-200 hover:bg-slate-800">Refresh</button>
          </div>

          <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="rounded-lg bg-[#0d1117] border border-slate-700 p-4">
              <dt class="text-xs text-slate-400 uppercase tracking-wide">Username</dt>
              <dd class="text-white font-medium mt-1">{{ user()!.username }}</dd>
            </div>
            <div class="rounded-lg bg-[#0d1117] border border-slate-700 p-4">
              <dt class="text-xs text-slate-400 uppercase tracking-wide">Email address</dt>
              <dd class="text-white font-medium mt-1 break-all">{{ user()!.email }}</dd>
            </div>
            <div class="rounded-lg bg-[#0d1117] border border-slate-700 p-4">
              <dt class="text-xs text-slate-400 uppercase tracking-wide">Account status</dt>
              <dd class="text-green-700 font-medium mt-1">Active</dd>
            </div>
            <div class="rounded-lg bg-[#0d1117] border border-slate-700 p-4">
              <dt class="text-xs text-slate-400 uppercase tracking-wide">User ID</dt>
              <dd class="text-white font-medium mt-1">#{{ user()!.id }}</dd>
            </div>
          </dl>
          <div class="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-700 pt-5">
            <div>
              <p class="text-sm font-medium text-white">Account security</p>
              <p class="text-xs text-slate-400">Password and profile editing require the account API.</p>
            </div>
